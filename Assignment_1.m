...
% -------------------------
wo = 60/(EEG.srate/2); bo = wo/35;  % wo = normalized center frequency (normalized w.r.t the Nyquist frequency , which in this case is EEG.srate/2)
                                    % EEG.srate = 128 ; So wo = 0.9375
                                    % bo = normalized width of the notch-filter
                                    % bo = 0.0267.
                                     
[bn,an] = designNotchPeakIIR(CenterFrequency= wo, Bandwidth= bo, Response="notch");   %algorithm is based on z-transform .
                                                                                      % an , bn ---> filter coefficients

data_filt = filtfilt(bn,an,data')';    %filtfilt ---> zero-phase filtering
                                       %filtfilt processes the data forward and then backward to cancel out any phase shift.
                                       %filtfilt works along the columns so
                                       % data' (transpose) is used
                                       % will need to more about how
                                       % filtering works
